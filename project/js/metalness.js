 ...
 */
const applyEnvMap = (mesh, materialNames, envMap, reflectivity, roughness, metalness) => {
  if (!mesh) {
    return
  }
...
const metalnessComponent =  {
  ...
  schema: {
    ...
    materials: {default: []},
    roughness: {default: 1, min: 0, max: 1},
    metalness: {default: 1, min: 0, max: 1},
  },
  ...
  init() {
    ...
    this.object3dsetHandler = () => {
      ...
      const {data} = this
      applyEnvMap(mesh, data.materials, this.texture, data.reflectivity, data.roughness, data.metalness)
    }
